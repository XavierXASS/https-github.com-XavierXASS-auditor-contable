...
#  Archivo: streamlit_app.py (bloque completo)
#  Versión: estable
# ==============================================
...
def check_auth():
    ...
    # Puedes definir en .streamlit/secrets.toml: MASTER_PASSWORD = "tu_password"
    master_from_secrets = st.secrets.get("MASTER_PASSWORD", None)
    st.caption("Consejo: si no tienes secrets configurados, puedes usar la clave de prueba: PERITO_EMAPAG_2025")

    pw = st.text_input("Acceso Maestra:", type="password")
    if st.button("DESBLOQUEAR"):
        ok = False
        if master_from_secrets:
            ok = (pw == master_from_secrets)
        else:
            ok = (pw == "PERITO_EMAPAG_2025")
        if ok:
...
def display_pdf(uploaded_file):
    ...
    # IFRAME CORRECTO (con <iframe ...> ... </iframe>)
    html = f'<iframe src="data:application/pdf;base64,{base64_pdf}" width="100%" height="840" style="border:none;"></iframe>'
...
if ex_file and pdf_files:
    ...
    m_index.success(f"⚡ PDFs indexados: {len(pdf_files)} archivo(s).")
    ...
    # ---- DOS COLUMNAS: VISOR (izq) + FORM (der) ----
